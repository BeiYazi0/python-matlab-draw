...
print('SoftSand RPM:increase in Vp after 10% porosity decrease={:.0%}'.format((vp_ssm[0]-vp_ssm[1])/vp_ssm[1]))
...
ip_rpt0, vpvs_rpt0 = rpt(model='stiff',vsh=0.6,fluid='oil',phic=0.4,Cn=8,P=45,f=.3)
...
def twolayer(vp0,vs0,rho0,vp1,vs1,rho1):
    ...
    ang=np.arange(31)
    wavelet=np.array(ricker(.25,0.001,25)).flatten()
    wavelet=wavelet[0:500]
    print(wavelet.shape)
...
# print('({:>4.0f}-{:<4.0f}m,{:3d} samples) Vp={:.0f}, Vs={:.0f}, rho={:.2f}, Ip={:.0f}, Vp/Vs={:.2f} '.format(z1,z2,samples_ss,vp1,vs1,rho1,
